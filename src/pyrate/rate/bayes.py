...
class TeamNode:
    ...
    def set_opponent_pointers(self, team_node_list):
        self.opponents = self.team.games.loc[self.team.games["TRAIN"], "OPP_IDX"].apply(
            team_node_list.__getitem__
        )
    ...
    def eval_move(self, rating_z):
        ...
        return sum(
            [
                np.log(p) if win else np.log(1.0 - p)
                for win, p in zip(self.win_flags, pwin_vals)
            ]
        )
...
class Bayes(rb.RatingSystm):
    ...
    def predict_win_probability(self, team1, team2, loc=None):
        ...
        return np.mean(
            [
                rating_func(rating1, rating2)
                for rating1, rating2 in zip(
                    self.team_nodes[idx1].rating_samp, self.team_nodes[idx2].rating_samp
                )
            ]
        )
...
if __name__ == "__main__":
    ...
    ratings = pd.DataFrame(
        {"rating": bayes.ratings}, index=[t.name for t in bayes.teams]
    )
