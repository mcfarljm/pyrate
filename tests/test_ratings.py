...
class ToyLeagueHyper(unittest.TestCase):
    def setUp(self):
        ...
        self.raw_df = pd.DataFrame(
            ...
            columns=["game_id", "team_id", "points", "location"],
        )
    ...
    def testScoreCap(self):
        ...
        lsq = leastsquares.LeastSquares(
            self.league, game_outcome_measure=gom.CappedPointDifference(1)
        )
        for rating, expected_rating in zip(lsq.df_teams["rating"], expected_ratings):
            self.assertAlmostEqual(rating, expected_rating)
    ...
    def testStrengtOfSchedule(self):
        ...
        lsq = leastsquares.LeastSquares(self.league)
        for sos_past, expected_sos in zip(
            lsq.df_teams["strength_of_schedule_past"], expected_sos_vals
        ):
    ...
    def testMLEStrengtOfSchedule(self):
        ...
        sol = mle.MaximumLikelihood(self.league, method=mle.Points())
        for sos_past, expected_sos in zip(
            sol.df_teams["strength_of_schedule_past"], expected_sos_vals
        ):
            self.assertAlmostEqual(sos_past, expected_sos)
...
class ToyLeagueGames(unittest.TestCase):
    ...
    def testOffenseDefense(self):
        ...
        lsq = leastsquares.LeastSquares(self.league)
        for offense, expected in zip(lsq.df_teams["offense"], expected_offense):
            self.assertAlmostEqual(offense, expected)
        for defense, expected in zip(lsq.df_teams["defense"], expected_defense):
            self.assertAlmostEqual(defense, expected)
...
class LeaveOneOutPredictions(unittest.TestCase):
    ...
    def leaveOneOutDriver(self, weight_func=None):
        ...
        def get_actual_LOO_pred(index):
            ...
            if (
                self.raw_df.loc[index, "team_id"]
                != lsq_full.single_games.loc[index, "team_id"]
            ):
                pred_gom = pred_gom * -1.0
            return pred_gom.iat[0]

        actual_loo_preds = pd.Series(
            [get_actual_LOO_pred(index) for index in self.raw_df.index],
...
class ToyLeagueScheduled(unittest.TestCase):
    ...
    def testSchedule(self):
        ...
        for team_id, expected_scheduled_count in zip(
            self.league.teams.index, expected_scheduled_counts
        ):
            self.assertEqual(
                sum(self.league.double_schedule["team_id"] == team_id),
