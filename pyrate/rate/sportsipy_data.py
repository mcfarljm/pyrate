...
def _get_team_games(Schedule, team_id, year):
    ...
    game_data = [(g.opponent_abbr, g.points_scored, g.points_allowed, g.datetime, g.location) for g in schedule]
    df = pd.DataFrame(game_data, columns=['opponent_id', 'points', 'opponent_points', 'date', 'location'])
